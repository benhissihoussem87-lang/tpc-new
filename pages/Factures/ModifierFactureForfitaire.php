...
$infosFacture    = $facture->detailFacture($numFacture);
$ProjetsFacture  = $facture->get_AllProjets_ByFacture($numFacture);
$bonCommandeClient = $bonCommande->getDetailBonCommandeByNumFacture($numFacture);
$adresseFacture  = '';
if (!empty($ProjetsFacture)) {
    $adresseFacture = $ProjetsFacture[0]['adresseClient'] ?? '';
}
$adresseOptions = $facture->get_All_AdressesClient_ProjetsFacture($numFacture);
...
// Handle submit (keep old button name for compatibility)
if (isset($_REQUEST['btnSubmitAjoutFactureForfitaire']) || isset($_REQUEST['btnSubmitModifier'])) {
    $Numero_Facture = $_POST['num_fact'];
    $numBonCommandeClient = isset($_POST['numboncommande']) ? trim($_POST['numboncommande']) : '';
    if (!isset($_POST['adresseClient']) || trim($_POST['adresseClient']) === '') {
        $_POST['adresseClient'] = isset($adresseFacture) ? $adresseFacture : '';
    }
    $targetAdresse = trim((string)$_POST['adresseClient']);

    // Update facture header
    $facture->Modifier(
        @$_POST['num_fact'],
        @$_POST['client'],
        @$_POST['numboncommande'],
        @$_POST['date'],
        @$_POST['reglement']
    );

    $verifFactureInProjetsFacture = $facture->VerifFacturedansFacturesProjet($_POST['num_fact'], $targetAdresse);
    $lineCount = max(
    ...
    if ($verifFactureInProjetsFacture) {
        // Delete and re-insert all lines for this address
        if ($facture->delete_All_Projets_By_FactureAndMultiAdress(@$_POST['num_fact'], $targetAdresse)) {
            for ($i = 0; $i < $lineCount; $i++) {
                ...
                if ($idp > 0 && $hasPrice) {
                    ...
                    // Keep mirrored offres aligned with facture lines
                    $offre->ModifierProjets_Offre(
                        @$_POST['num_fact'],
                        @$_POST['prix_unit_htv'][$i],
                        @$_POST['qte'][$i],
                        @$_POST['tva'][$i],
        ...
        // Insert fresh lines for this address
        for ($i = 0; $i < $lineCount; $i++) {
            $idp = isset($_POST['idProjet'][$i]) && is_numeric($_POST['idProjet'][$i]) ? (int)$_POST['idProjet'][$i] : 0;
            $hasPrice = (!empty($_POST['prix_unit_htv'][$i]) || !empty($_POST['prixForfitaire'][$i]) || !empty($_POST['prixTTC'][$i]));
            if ($idp > 0 && $hasPrice) {
    ...
    // Redirects similar to standard modifier flow
    if ($_POST['reglement'] === 'oui') {
    ...
    } elseif (!empty($_POST['numboncommande'])) {
        ...
        echo "<script>document.location.href='main.php?Factures&modifierForfitaire=$numFacture'</script>";
    }
}
?>
...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        <h4 class="mb-3">Modifier Facture Forfaitaire</h4>
        <form method="post">
            <div class="modal-body row">
                <div class="mb-3 col-2">
                    <label for="num_fact" class="col-form-label">No. Facture:</label>
                    <input type="text" value="<?=htmlspecialchars($infosFacture['num_fact'] ?? '')?>" class="form-control" id="num_fact" name="num_fact"/>
                </div>
                    ...
                    <select class="form-control" id="client" name="client">
                        <?php if (!empty($clients)) {
                            foreach ($clients as $key) {
                                $selected = ($infosFacture['nom_client'] ?? '') === ($key['nom_client'] ?? '') ? 'selected' : '';
                                ?>
                                <option value="<?=$key['id']?>" <?=$selected?>><?=$key['nom_client']?></option>
                        <?php }} ?>
                    </select>
                </div>
                <div class="mb-3 col-2">
                    <label for="numboncommande" class="col-form-label">Bon.Commande Client:</label>
                    <input type="text" value="<?=htmlspecialchars($bonCommandeClient['num_bon_commandeClient'] ?? '')?>" class="form-control" id="numboncommande" name="numboncommande"/>
                </div>
                <div class="mb-3 col-3">
                    <label for="date" class="col-form-label">Date Facture:</label>
                    <input type="date" value="<?=htmlspecialchars($infosFacture['date'] ?? '')?>" required class="form-control" id="date" name="date"/>
                </div>
                <div class="mb-3 col-2">
                ...
                <div class="mb-3 col-12">
                    ...
                    <input type="text" class="form-control" id="adresseClient" name="adresseClient" list="adresseOptions" value="<?=htmlspecialchars($adresseFacture)?>"/>
                    <datalist id="adresseOptions">
                        <?php if (!empty($adresseOptions)) {
                            foreach ($adresseOptions as $addr) {
                                $val = trim((string)($addr['adresseClient'] ?? ''));
                                if ($val === '') { continue; }
                                ?>
                ...
                <div id="factureProjectsContainer">
                    <?php foreach ($factureProjectLines as $line) {
                        ...
                        <div class="row g-2 align-items-end mb-2 projet-line" data-line>
                            ...
                            <div class="col-md-2">
                                <label class="form-label">Prix Forfaitaire</label>
